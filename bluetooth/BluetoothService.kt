...
class BluetoothService : Service() {
    ...
    @ExperimentalCoroutinesApi
    @InternalCoroutinesApi
    fun connected(btDevice: BluetoothDevice) = mapManagerAndDevice.containsKey(btDevice)
    ...
    override fun onDestroy() {
        for ((device, btManager) in mapManagerAndDevice){
            CoroutineScope(Dispatchers.IO).launch { btManager.disconnect() }
        }
        stopForeground(true)
        stopSelf()
    }
